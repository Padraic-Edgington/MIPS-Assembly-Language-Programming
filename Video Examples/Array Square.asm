Square:
		lw		$t0, 0 ($a0)		#  Length of the array
		
		li		$t1, 0				#  Counter
loop:	bge		$t1, $t0, end_loop
		
#		li		$t4, 4
#		mul		$t2, $t1, $t4		#  Multiply index by 4 to get byte offset.
#		addi	$t2, $t2, 4			#  Add 4 to skip the length field.
#		add		$t2, $a0, $t2		#  Location of element $t1 in array $a0.
		
#		lw		$t3, 0 ($t2)		#  Fetch element $t1.
#		mul		$t3, $t3, $t3
#		sw		$t3, 0 ($t2)		#  Store squared element.
		
		sll		$t2, $t1, 2			#  Multiply by 4.
		add		$t2, $a0, $t2		#  Memory address of element $t1 - 1.
		
		lw		$t3, 4 ($t2)		#  Fetch element $t1.
		mul		$t3, $t3, $t3
		sw		$t3, 4 ($t2)		#  Store squared element.
...
test1b:	.asciiz	"[5] => [25]"
		...
		.data
test2:	.word	3,  2,  4,  6
test2a:	.word	3,  4, 16, 36
test2b:	.asciiz	"[2 4 6] => [4 16 36]"
		.text
		...
		jal		Assert_Equal_Array
		.data
test3:	.word	4,  1,  3,  5,  7
test3a:	.word	4,  1,  9, 25, 49
test3b:	.asciiz	"[1 3 5 7] => [1 9 25 49]"
		.text
		...
		jal		Assert_Equal_Array
		.data
test4:	.word	7,   5,   2,   7,  19,  12,   9,   2
test4a:	.word	7,  25,   4,  49, 361, 144,  81,   4
test4b:	.asciiz	"[5 2 7 19 12 9 2] => [25 4 49 361 144 81 4]"
		.text
